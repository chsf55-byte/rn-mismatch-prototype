...
- **Make the dollar impact of mismatches visible and reportable** - not just to the internal team, but as proof to restaurant buyers that Supy pays for itself (see section 8, Business metrics)

**Non-goals** *(detailed reasoning in section 5)*
...
- Per-SKU or per-supplier tolerance configuration - v1 ships one global tolerance ("global" = not per-SKU/per-supplier; it still has two dimensions, a quantity % and a price %, configured independently)
...
- **A single global tolerance is an acceptable starting point** for every customer in v1, even though real customers will eventually want per-supplier or per-category thresholds. ("Global" means the same thresholds apply platform-wide - there are still two independent dimensions, one quantity % and one price %)
...
- They often hold context the reviewer doesn't ("driver said the rest is coming tomorrow," "box was visibly damaged") - so they need a lightweight way to attach a note, and later, **to be notified how it was resolved** (see section 6.2)

So this PRD designs a **two-role workflow**: operators surface and contextualize mismatches as a byproduct of the receiving flow they already do; a designated reviewer works a dedicated queue to resolve them. *(Which specific title - procurement vs. accounts - holds resolution authority is a per-customer question; assumed to map to an existing role, per section 3.)*
...
- A **severity tier** (Small / Medium / Critical) per mismatch, driven by dollar impact, so the queue can be scanned by what matters most, not just sorted by date (section 6.1)
- A **Mismatch Queue**: one place a reviewer works from, across branches and suppliers
- A **notification** the moment a mismatch is flagged or escalated, so a reviewer doesn't have to remember to go check (section 6.2)
...
- **Four resolution actions**, applied per line item (a GRN can have some lines resolved and others still open), split cleanly by what kind of decision each one actually is:
  - **Settle at received quantity** - for a **pure quantity variance only** (price matches confirmed): the corrected line total (received qty × the already-agreed unit price) is **calculated automatically and locked**, not typed in. Covers a shortfall being written off, or an overage being kept
  - **Return surplus** - over-delivery only, and likewise only when price matches confirmed: settle at the confirmed quantity instead, i.e. pay only for what was ordered. Also auto-calculated and locked
  - **Record debit/credit note** - the one action with a manually entered amount, because it's the only one with genuine judgment in it: offered whenever the price varies, and it is the **sole resolution action (besides Escalate) when both quantity and price are off** - one note covers the line's full dollar impact. A **credit note** when the business was overcharged (supplier owes money back); a **debit note** when the business was undercharged (owes the supplier more). The suggested amount defaults to the system-calculated variance and is editable, since real negotiations rarely land on the exact calculated number
  - **Escalate** - hand the line to the **Accounts Manager** for a second opinion, with a note on why. Single fixed destination, not a pick-list - see section 6.9 on why
...
| **Operator-side resolution** | Resolution authority stays with the reviewer, even for small variances - see section 4. If reviewers end up rubber-stamping trivial cases, raising the tolerance is the first lever, not delegating approval down. |

### Edge cases handled

| Case | Handling |
|---|---|
| Under-delivery (received < confirmed qty, price as confirmed) | Flagged if outside tolerance; resolved via **Settle at received quantity** - the corrected total is arithmetic (received qty × confirmed price), so it's calculated and locked, not typed in |
| Over-delivery (received > confirmed qty, price as confirmed) | Flagged the same way - **paying for unordered stock is its own liability**, not free upside. Two valid outcomes, both auto-calculated: **Settle at received quantity** (keep the extra, pay for it) or **Return surplus** (pay only for the confirmed amount; physical pickup is arranged off-platform, same as any supplier coordination) |
...
| Both qty **and** price off on the same line | Both variances shown together, but resolved with **one action: Record debit/credit note**, whose amount covers the line's full dollar impact (quantity and price combined). The auto-settle actions are deliberately not offered here - a locked arithmetic total can't absorb a price judgment, and splitting the line into two decisions would break the one-action-per-line model |
...
**Severity is derived from current thresholds while a line is open, and snapshotted onto the record the moment the line is resolved or escalated.** Changing thresholds therefore re-tiers only still-open lines; closed lines keep the tier they had when the decision was made, so history never silently rewrites itself.
...
A screen scoped to whatever the reviewer has visibility over (branches/suppliers, per existing permissions). Each row: supplier, branch, GRN date, lines flagged, **severity**, **$ exposure**, age, status. Exposure is the **sum of absolute line impacts** - signed variances are never netted, so a -$30 line and a +$30 line can't hide each other. A row's severity is the tier of the GRN's total exposure (individual lines still carry their own tier in the detail view). Sortable/filterable - because the two things a reviewer triages by are **how much money, and how long it's been sitting.**
...
- **Line status:** `Open` → `Resolved` (terminal), or `Open` → `Escalated` → `Resolved`. **Escalation is recorded separately from the resolution**, so resolving a previously escalated line keeps both entries in the audit trail - the escalation is never overwritten by the final decision.
...
- **Mandatory note on every action, including auto-calculated ones.** A resolution's "why" is worthless if it's optional - see section 6.5.
- **The debit/credit note captures intent, not the accounting artifact.** Keeps this feature inside Supy's boundary instead of turning it into an integrations project - see section 5.
- **A global tolerance and severity scale, not per-item.** "Global" means the thresholds apply platform-wide rather than per-SKU/per-supplier - there are still two independent tolerance dimensions (quantity % and price %). Per-item granularity is more correct in theory, but it's config surface with no evidence yet that customers need it. Shipping the simple version first is how we find out.
...
*Prototype scope note: the prototype demonstrates the reviewer flow end to end. Two receiving-flow concerns are stubbed rather than built: operator note **capture** (notes appear as seeded data; entry happens at GRN submission, a screen the prototype doesn't include) and event-driven flag persistence (the prototype recomputes flags live from the current tolerance, so tightening/loosening it re-evaluates open lines - production stores the flag at submission per the story below).*
...
| As a **reviewer**, I want every flagged mismatch to show a severity tier based on $ impact, so I can tell what actually matters without doing the math myself. | • Each flagged line shows Small/Medium/Critical from configured $ thresholds.<br>• Thresholds are independent of tolerance %.<br>• Changing thresholds re-evaluates only still-open lines; resolved/escalated lines keep their historical tier. | Deliberately dollar-based, not percentage-based - a small % on a large order can matter more than a large % on a $2 item. | Severity is derived at read time from stored $ impact + current threshold config **while a line is open**, then snapshotted onto the resolution/escalation record when the line closes - so threshold changes apply to items still sitting open, but never rewrite the tier a closed decision was made under. |
| As a **reviewer**, I want one queue showing every open mismatch across branches and suppliers, sorted by severity then age, so I can triage without hunting through individual GRNs. | • Queue lists every GRN with ≥1 flagged line, scoped to the reviewer's permitted branches/suppliers.<br>• Default sort: worst severity first, then oldest first within a tier.<br>• Filterable by status, searchable by supplier/branch/GRN ID. | Age and $ impact are the two things a reviewer actually triages by - the sort reflects that instead of defaulting to recency. | Needs an indexed view joining lines by status + severity + branch scope. Pagination isn't modeled in the v1 prototype but will matter at real customer volume. |
...
- **Average $ recovered per credit-note resolution** (numerator and denominator both scoped to credit notes, so the average isn't diluted by arithmetic settles) - gives customer success a concrete ROI number per account.
...
**Leading indicator before there's volume:** in the first two weeks, whether reviewers open the queue at all without being prompted. If nobody's checking it unprompted, the notification path (section 6.2) - not the resolution flow itself - is the thing to fix first.
