...
*(20-second summary for anyone who won't read past this section.)*
...
- **Make the dollar impact of mismatches visible and reportable** — not just to the internal team, but as proof to restaurant buyers that Supy pays for itself (see §7, Business metrics).

**Non-goals** *(detailed reasoning in §5)*
...
This PRD is written on top of a few things I'm taking as given rather than redesigning:
...
- They often hold context the reviewer doesn't ("driver said the rest is coming tomorrow," "box was visibly damaged") — so they need a lightweight way to attach a note, and later, **to be notified how it was resolved** (see §6.2).

So this PRD designs a **two-role workflow**: operators surface and contextualize mismatches as a byproduct of the receiving flow they already do; a designated reviewer works a dedicated queue to resolve them. *(Which specific title — procurement vs. accounts — holds resolution authority is a per-customer question; assumed to map to an existing role, per §3.)*
...
- A **severity tier** (Small / Medium / Critical) per mismatch, driven by dollar impact, so the queue can be scanned by what matters most, not just sorted by date (§6.1).
- A **Mismatch Queue**: one place a reviewer works from, across branches and suppliers.
- A **notification** the moment a mismatch is flagged or escalated, so a reviewer doesn't have to remember to go check (§6.2).
...
- **Four resolution actions**, applied per line item (a GRN can have some lines resolved and others still open), split cleanly by what kind of decision each one actually is:
  ...
  - **Escalate** — hand the line to the **Accounts Manager** for a second opinion, with a note on why. Single fixed destination, not a pick-list — see §6.9 on why.
...
| **Operator-side resolution** | Resolution authority stays with the reviewer, even for small variances — see §4. If reviewers end up rubber-stamping trivial cases, raising the tolerance is the first lever, not delegating approval down. |
...
- **Mandatory note on every action, including auto-calculated ones.** A resolution's "why" is worthless if it's optional — see §6.5.
- **The debit/credit note captures intent, not the accounting artifact.** Keeps this feature inside Supy's boundary instead of turning it into an integrations project — see §5.
...
**Leading indicator before there's volume:** in the first two weeks, whether reviewers open the queue at all without being prompted. If nobody's checking it unprompted, the notification path (§6.2) — not the resolution flow itself — is the thing to fix first.

---

## 8. Open questions for the team

- Does "reviewer" map to an existing Supy role/permission today, or do we need a new permission bit? (Assumed yes, per §3 — needs confirming with the platform/permissions owner.)
...
- What channel should the notifications in §6.2 ride on — existing in-app notifications, email, or both? (Assumed in-app for v1, pending what Supy already has.)
